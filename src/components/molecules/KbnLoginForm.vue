...
export default {
    ...
    computed: {
        ...
        valid () {
            ...
            const fields = Object.keys(validation) // Dictionaryを作成
            let valid = true
            for (let i = 0; i < fields.length; i++) {
                const field = fields[i] // 各フィールドを取り出し
                valid = Object.keys(validation[field]).every(key => validation[field][key]) // 各フィールドの各値を分離、全てtrueか確認
                if (!valid) { break } // validでなければ終了
            }
            return valid
        },

        disableLoginAction () {
            return !this.valid || this.progress // validでないか、進んでいるか？
        },
    },
    ...
    methods: {
        ...
        async handleClick (ev) {
            if (this.disableLoginAction) {return} // 不備があればログインできない
            this.progress = true
            this.error = ''
            await this.$nextTick()
